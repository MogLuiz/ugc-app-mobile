...
type Props = {
  detailHrefBase: '/(creator)/mensagens' | '/(business)/mensagens'
  emptyTitle: string
...
function EmptyInitial({ title, description }: { title: string; description: string }) {
  return (
...
export function ChatConversationListScreen({
  detailHrefBase,
  emptyTitle,
  emptyDescription,
}: Props) {
  ...
  function handlePress(conversationId: string) {
    router.push(`${detailHrefBase}/${conversationId}` as never)
  }
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {isFirstLoad ? (
        ...
        <>
          ...
          {!hasAnyConversations ? (
            <EmptyInitial title={emptyTitle} description={emptyDescription} />
          ) : filtered.length === 0 ? (
