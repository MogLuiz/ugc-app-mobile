...
function makeLocalMessage(content: string, conversationId: string, userId: string): LocalMessage {
  return {
...
export function ChatThreadScreen() {
  ...
  const lastFailedMessage = [...localMessages].find((m) => m.deliveryStatus === 'failed') ?? null
      ...
      >
        {isInitialLoading ? (
          ...
          <View style={styles.centerContent}>
            <Text style={styles.errorTitle}>Não foi possível carregar a conversa.</Text>
            <Pressable onPress={() => void messagesQuery.refetch()} style={styles.retryLoadButton}>
              <Text style={styles.retryLoadText}>Tentar novamente</Text>
            </Pressable>
          </View>
        ) : (
          <FlatList
            ...
            contentContainerStyle={[
              styles.listContent,
              { paddingBottom: Math.max(insets.bottom, 10), paddingTop: 12 },
            ]}
            onEndReached={() => {
            ...
            ListFooterComponent={
              messagesQuery.isFetchingNextPage ? (
                <ActivityIndicator style={styles.fetchMoreSpinner} color={colors.primary} />
              ) : null
            }
            ListEmptyComponent={
              <View style={styles.centerContent}>
        ...
        <MessageInput
          onSend={handleSend}
          onRetryLastFailed={
            lastFailedMessage ? () => void handleRetry(lastFailedMessage.id) : undefined
          }
